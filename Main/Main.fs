...
let main argv =
    ...
    let filteredOrders =
        let statusFilter =
            if askYesNo "Filter by status?" then askStatus () else None

        let originFilter =
            if askYesNo "Filter by origin?" then askOrigin () else None
    ...
    let orderAveragesByMonthYear = HelperFunctions.calculation.CalculateAverageAmountByMonthYear itemsList filteredOrders
    let taxesAveragesByMonthYear = HelperFunctions.calculation.CalculateAverageTaxesByMonthYear itemsList filteredOrders
    ...
    Writer.writeAveragesByMonthYear outputPathByMonthYear orderAveragesByMonthYear taxesAveragesByMonthYear
    printfn "CSV saved at: %s" outputPath

    if askYesNo "Save to local database (SQLite)?" then
        Writer.writeOrderTotalsToSqlite dbPath orderTotals
        printfn "SQLite saved at: %s" dbPath
